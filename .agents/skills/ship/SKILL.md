---
name: ship
description: Lint, build, test, verify specs, commit, and push — one micro-commit to remote. Use when work is complete and ready to ship safely.
---
...
For every changed file in `src/lib/`, check whether a related spec exists under `specs/`. Use the module name (grid, physics, scoring, rendering, track-editor, track-format) to locate relevant specs.
...
2. Draft a commit message:
   ...
   - Scope should be the module name: `grid`, `physics`, `scoring`, `renderer`, `ui`, `track-format`, `infra`
   - Keep the subject line under 72 characters
   - Match the style of recent commits from Step 1
   - If the user provided a message via `$ARGUMENTS`, use it as the basis
   - Add `Co-Authored-By: Antigravity <noreply@anthropic.com>` trailer
3. Create the commit.
